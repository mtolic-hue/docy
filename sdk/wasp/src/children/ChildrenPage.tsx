...
import { Child } from "wasp/entities";
import { Link } from "wasp/client/router";
import { Button } from "../shared/components/Button";
import { Input } from "../shared/components/Input";

type FormValues = { name: string; birthDate?: string; notes?: string };
        ...
        <section className="card w-full max-w-3xl p-4">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
            <Controller
              ...
              control={control}
              rules={{ required: true }}
              render={({ field, fieldState }) => <Input label="Name" fieldState={fieldState} {...field} />}
            />
            <Controller
              name="birthDate"
              control={control}
              render={({ field, fieldState }) => <Input label="Birth date (optional)" fieldState={fieldState} type="date" {...field} />}
            />
            <Controller
              name="notes"
              control={control}
              render={({ field, fieldState }) => <Input label="Notes (optional)" fieldState={fieldState} {...field} />}
            />
          ...
          <ul className="flex flex-col gap-2">
            {children && children.length > 0 ? (
              children.map((c: Child) => (
                <li key={c.id}>
                  <Link to={({ pathname: `/children/${c.id}` } as any) as any} className="text-primary-600">
                    {c.name}
                  </Link>
                </li>
              ))
