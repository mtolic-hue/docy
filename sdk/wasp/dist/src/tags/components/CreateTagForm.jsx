...
export function CreateTagForm({ onTagCreated }) {
    ...
    return (<form id={CREATE_TAG_FORM_ID} onSubmit={stopPropagate(handleSubmit(onSubmit))} className="flex flex-col gap-6">
      <Controller name="name" control={control} rules={{
            required: { value: true, message: "Name is required" },
        }} render={({ field, fieldState }) => (<Input label="Name" placeholder="Enter tag name" fieldState={fieldState} {...field}/>)}/>
      <ColorRadioButtons color={color} setColor={(color) => setValue("color", color)}/>
      {name && (<div className="flex flex-col gap-2">
          <span className="label">Preview</span>
          <div className="flex flex-wrap gap-2">
            <TagLabel tag={{ id: "", name, color }} isActive={true} showColorCircle/>
            <TagLabel tag={{ id: "", name, color }} isActive={false} showColorCircle/>
          </div>
        </div>)}
    </form>);
}
...
//# sourceMappingURL=CreateTagForm.jsx.map