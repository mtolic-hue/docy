...
export function CreateTaskForm() {
    ...
    const { handleSubmit, getValues, setValue, watch, control, reset } = useForm({
        defaultValues: {
            description: "",
            tagIds: [],
        },
    ...
    const toggleTag = React.useCallback(function toggleTag(id) {
    ...
    return (<form onSubmit={handleSubmit(onSubmit)} className="flex w-full flex-col gap-6" id="create-task">
      <h2 className="text-xl font-semibold">Create a new task</h2>
      <Controller name="description" control={control} rules={{
            required: { value: true, message: "Description is required" },
        }} render={({ field, fieldState }) => (<Input label="Description" placeholder="What do I need to do?" fieldState={fieldState} {...field}/>)}/>
...
//# sourceMappingURL=CreateTaskForm.jsx.map